...
public class CRC16Util {
    ...
    public static short calcCrc16(byte[] data, int offset, int len, int preval) {
        ...
        return (short) (((ucCRCHi & 0x00ff) << 8) | (ucCRCLo & 0x00ff) & 0xffff);
    }
...
//            Log.i("BLUEDATA", "crc ---- : " + substring + "  " + substring1);
     ...
     * 华为雷达2.4认证 32位雷达认证
     *
     * @param username 登录雷达用户名
     * @param password 登录雷达用户名密码
     * @param nonce    2.3返回的Nonce数组 (长度为16)
     * @return 返回32位认证数组
     */
    public static byte[] huaweiLidaSha256(String username, String password, byte[] nonce) {
        int userLength = username.length();
        int passwordLength = password.length();
        byte[] userPassword = new byte[userLength + passwordLength + nonce.length + 2];

        for (int i = 0; i < userLength; i++) {
            userPassword[i] = Convert.intToByte(username.charAt(i));
        }
        userPassword[userLength] = (byte) ':';

        for (int i = 0; i < passwordLength; i++) {
            userPassword[userLength + 1 + i] = Convert.intToByte(password.charAt(i));
        }
        userPassword[userLength + passwordLength + 1] = (byte) ':';

        for (int i = 0; i < nonce.length; i++) {
            userPassword[userLength + passwordLength + 2 + i] = nonce[i];
        }
        byte[] temArr = new byte[userPassword.length * 1000];
        for (int i = 0; i < 1000; i++) {
            System.arraycopy(userPassword, 0, temArr, i * userPassword.length, userPassword.length);
        }
        byte[] sha256;
        try {
            MessageDigest messageDigest = MessageDigest.getInstance("SHA-256");
            sha256 = messageDigest.digest(temArr);
            return sha256;
